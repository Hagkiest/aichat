...
class MainWindow(QMainWindow,Ui_MainWindow):
    ...
    def create_widget(self):
        ...
        if self.sum % 2:  # 根据判断创建左右气泡
            widget = Set_question.set_return(self, self.icon, self.text,
                                             QtCore.Qt.LeftToRight)  # 调用new_widget.py中方法生成左气泡
            QApplication.processEvents()  # 等待并处理主循环事件队列
            self.show_text_animation(widget)
        else:
            widget = Set_question.set_return(self, self.icon, self.text,
                                             QtCore.Qt.RightToLeft)  # 调用new_widget.py中方法生成右气泡
            QApplication.processEvents()  # 等待并处理主循环事件队列
            self.show_text_animation(widget)

    def show_text_animation(self, widget):
        text = self.text
        for i in range(1, len(text) + 1):
            partial_text = text[:i]
            widget.findChild(QTextBrowser).setPlainText(partial_text)
            QApplication.processEvents()
            # 使用QTimer代替QThread.msleep()
            QTimer.singleShot(50, lambda: None)  # 暂停100毫秒
